...
#' @param nbrr  Number of BRR samples to
...
doBRR <- function(df, fit, nbrr, us = 1:30, uPSU=1:2, strata=NULL, PSU=NULL, weights=NULL){
    if(!inherits(fit, "gam")){
        stop("Regression model supplied to fit must be of class \"gam\" ")
    }
    if("strata_PSU" %in% colnames(df)) {
        message("strata_PSU column detected in dataframe used for model fitting. This column will be used for BRR.")
    } else {
        df$strata_PSU <- paste0(df[[strata]],"_",df[[PSU]])
    }
    n_us <- length(us)
    # get the hadamard matrix
    H_brr <- matrix(paste0(rep(us, nbrr), "_", sample(uPSU, n_us*nbrr, replace=TRUE)), n_us, nbrr, byrow=FALSE)
    ...
    pb <- utils::txtProgressBar(min=0, max=nbrr,style=3)
    for(b in 1:nbrr){
        ...
        fit_brr <- mgcv::gam(fit$formula, data=data_brr,
                       family=fit$family$family,
                       method=fit$method,weights=weights_b)
